import React, { useState } from 'react';
import { Button } from '@/components/ui/button';
import { Bookmark } from 'lucide-react';

const HomeCard = ({
  relevancePercentage,
  citationCount,
  title,
  description,
  authors = [],
  downloadable = false,
  onSavePaper,
  onDownload,
  initialSaved = false,
  className = ""
}) => {
  const [isSaved, setIsSaved] = useState(initialSaved);
  const [isLoading, setIsLoading] = useState(false);

  // Determine badge color based on relevance percentage
  const getBadgeColor = (percentage) => {
    if (percentage >= 95) {
      return {
        background: 'rgba(16, 183, 127, 0.15)', // Accent color with transparency
        text: '#10b77f', // Accent color
        border: '#10b77f'
      };
    } else if (percentage >= 90) {
      return {
        background: 'rgba(245, 158, 11, 0.15)', // Warning color with transparency
        text: '#f59e0b',
        border: '#f59e0b'
      };
    } else {
      return {
        background: 'rgba(239, 68, 68, 0.15)', // Red warning with transparency
        text: '#ef4444',
        border: '#ef4444'
      };
    }
  };

  const handleSavePaper = async () => {
    if (isLoading) return; // Prevent multiple clicks
    
    setIsLoading(true);
    
    if (onSavePaper) {
      try {
        await onSavePaper();
        setIsSaved(prev => !prev); // Toggle saved state
      } catch (error) {
        console.error('Error saving paper:', error);
      }
    } else {
      setIsSaved(prev => !prev); // Toggle saved state
    }
    
    setIsLoading(false);
  };

  const handleDownload = () => {
    if (downloadable && onDownload) {
      onDownload();
    }
  };

  const badgeColors = relevancePercentage ? getBadgeColor(relevancePercentage) : null;

  return (
    <div className={`bg-white border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow duration-200 ${className}`}>
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-4">
          {/* Relevance Badge */}
          {relevancePercentage && (
            <span
              className="px-3 py-1 text-sm font-medium rounded-full border"
              style={{
                backgroundColor: badgeColors.background,
                color: badgeColors.text,
                borderColor: badgeColors.border
              }}
            >
              {relevancePercentage}% Relevance
            </span>
          )}
          
          {/* Citation Count */}
          {citationCount && (
            <span className="text-gray-500 text-sm">
              {citationCount.toLocaleString()} Citation{citationCount !== 1 ? 's' : ''}
            </span>
          )}
        </div>
      </div>

      {/* Title */}
      <h3 className="text-xl font-semibold text-gray-900 mb-3 leading-tight">
        {title}
      </h3>

      {/* Description */}
      <p className="text-gray-600 text-sm leading-relaxed mb-4">
        {description}
      </p>

      {/* Save Paper Button */}
      <div className="mb-6">
        <Button
          onClick={handleSavePaper}
          disabled={isLoading}
          className={`transition-all duration-200 ${
            isSaved 
              ? 'bg-green-600 hover:bg-green-700 text-white' 
              : 'text-white hover:opacity-90'
          } ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
          style={{
            backgroundColor: isSaved ? '#10b77f' : '#3c83f6'
          }}
        >
          <Bookmark 
            className={`h-4 w-4 mr-2 ${isSaved ? 'fill-current' : ''}`} 
          />
          {isLoading ? 'Processing...' : isSaved ? 'Saved Paper' : 'Save Paper'}
        </Button>
      </div>

      {/* Footer */}
      <div className="flex items-center justify-between">
        {/* Authors */}
        <div className="flex-1">
          <p className="text-gray-500 text-sm">
            {authors.length > 0 ? authors.join(', ') : 'Authors Name'}
          </p>
        </div>

        {/* Download Icon */}
        <div className="flex-shrink-0">
          <button
            onClick={handleDownload}
            disabled={!downloadable}
            className={`p-2 rounded-md transition-colors duration-200 ${
              downloadable 
                ? 'text-gray-600 hover:text-gray-900 hover:bg-gray-100 cursor-pointer' 
                : 'text-gray-300 cursor-not-allowed'
            }`}
            title={downloadable ? 'Download paper' : 'Download not available'}
          >
            {downloadable ? (
              // Can download icon
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M8 11.575C7.86667 11.575 7.74167 11.5543 7.625 11.513C7.50833 11.4717 7.4 11.4007 7.3 11.3L3.7 7.7C3.5 7.5 3.404 7.26667 3.412 7C3.42 6.73334 3.516 6.5 3.7 6.3C3.9 6.1 4.13767 5.996 4.413 5.988C4.68833 5.98 4.92567 6.07567 5.125 6.275L7 8.15V1C7 0.71667 7.096 0.479337 7.288 0.288004C7.48 0.0966702 7.71733 0.000670115 8 3.44827e-06C8.28267 -0.000663218 8.52033 0.0953369 8.713 0.288004C8.90567 0.48067 9.00133 0.718003 9 1V8.15L10.875 6.275C11.075 6.075 11.3127 5.979 11.588 5.987C11.8633 5.995 12.1007 6.09934 12.3 6.3C12.4833 6.5 12.5793 6.73334 12.588 7C12.5967 7.26667 12.5007 7.5 12.3 7.7L8.7 11.3C8.6 11.4 8.49167 11.471 8.375 11.513C8.25833 11.555 8.13333 11.5757 8 11.575ZM2 16C1.45 16 0.979333 15.8043 0.588 15.413C0.196666 15.0217 0.000666667 14.5507 0 14V12C0 11.7167 0.0960001 11.4793 0.288 11.288C0.48 11.0967 0.717333 11.0007 1 11C1.28267 10.9993 1.52033 11.0953 1.713 11.288C1.90567 11.4807 2.00133 11.718 2 12V14H14V12C14 11.7167 14.096 11.4793 14.288 11.288C14.48 11.0967 14.7173 11.0007 15 11C15.2827 10.9993 15.5203 11.0953 15.713 11.288C15.9057 11.4807 16.0013 11.718 16 12V14C16 14.55 15.8043 15.021 15.413 15.413C15.0217 15.805 14.5507 16.0007 14 16H2Z" fill="currentColor"/>
              </svg>
            ) : (
              // Can't download icon
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M4 17V19C4 19.5304 4.21071 20.0391 4.58579 20.4142C4.96086 20.7893 5.46957 21 6 21H18C18.3875 21.0003 18.7668 20.8879 19.0917 20.6767C19.4165 20.4654 19.6731 20.1643 19.83 19.81M7 11L12 16M12 16L14 14M12 16V12M16 12L17 11M12 4V8M3 3L21 21" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
            )}
          </button>
        </div>
      </div>
    </div>
  );
};

export default HomeCard;